...
int main(void)
{
    ...
    read(STDIN_FILENO, &lib1_process_cap, sizeof(pcap));
    
    /////////////////////////////PREPARE TARGET REGISTER FOR CCALL /////////////////////////////#
    printf("App: PID: %d\n", app_pid);
    printf("App: Received PID=%ld, PC=0x%lx, MAC=0x%lx\n", lib1_process_cap.PID, lib1_process_cap.PC, lib1_process_cap.MAC);
    ...
    asm volatile(
        ...
        :                     // No output operands
        : [var] "m" (lib1_process_cap.PID)  // Input operand (memory reference to stack_variable)
    ...
    asm volatile(
        ...
        :                     // No output operands
        : [var] "m" (lib1_process_cap.PC)  // Input operand (memory reference to stack_variable)
        : "x0"                // Clobbers (x1 register is modified)
    );
    ...
    int sum=0;
    printf("App1: calling Lib1\n");
    sum=syscall(SYS_pcall, lib1_process_cap.PID, lib1_process_cap.PC);
    printf("App1: Lib1 returned with result:%d\n",sum);
